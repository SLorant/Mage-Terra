...
import { useStore, usePlayerStore } from "../../components/useStore";
import VictoryScreen from "./components/VictoryScreen";
import { DominoSetter } from "./components/boardcomponents/DominoSetter";
...
export default function Game() {
    ...
    return onValue(playersRef, (snapshot) => {
      ...
      if (data) {
        ...
        snapshot.forEach((userSnapshot) => {
          ...
          const squaresData: SquareState[] = userSnapshot
            .child("Squares")
            .val();
          ...
          if (nameData)
            addPlayerInfo(playerId, nameData, scoreData, avatarData);
        });
      } else {
        setTimeout(() => {
          setLoading(false);
  ...
  const addPlayerInfo = (
    ...
    setPlayerInfos((prevPlayerInfos) => {
      ...
      const sortedPlayerInfosArray = Object.entries(updatedPlayerInfos).sort(
        ([, a], [, b]) => b.score - a.score
      );
  ...
  return (
    <main className="flex h-screen mainbg items-center justify-center font-sans relative">
      <img
        className="h-screen hidden lg:block lg:absolute top-0 left-0"
        ...
        <div id="gameContainer" className="relative">
          ...
          >
            <div className="absolute flex flex-col justify-start items-center top-0 lg:top-6 left-0 w-full h-full lg:h-[650px] bg-[#130242]">
              <div className="md:absolute md:-bottom-14 z-50 mt-[35px] md:right-44 md:mt-8 text-2xl text-white">
                <div className="relative">
                  ...
                  >
                    <path
                      d="M58.5 55L0.909314 0.250001L116.091 0.250011L58.5 55Z"
                      fill="#E1DAFF"
                    />
                  </svg>
                  <p className="absolute text-3xl top-0 left-10 text-darkblue z-50">
                    {countdown}
                  </p>
                </div>
              </div>
            ...
            <div className=" mt-28 md:mt-[600px] lg:mt-12  flex items-center justify-center  bg-purple-700 h-[560px] mb-4 gap-0 shadow-md">
              <DndProvider
                backend={window.innerWidth < 640 ? TouchBackend : HTML5Backend}
              >
                <Board
                  uniqueId={uniqueId}
                  room={room ?? ""}
      ...
      {!isPlayer && (
        <div className="xl:w-1/3 h-1/3 w-full md:w-1/2 absolute z-50 darkbg rounded-sm flex flex-col justify-around items-center">
          <h2 className="px-8 text-center text-2xl xl:text-3xl mt-8  text-white ">
            {loading
              ? "Loading..."
              : "The game has already started or does not exist"}
          </h2>
          <div className="flex">
            {!loading && (
              <button
