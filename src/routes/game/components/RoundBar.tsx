import { RoundBarProps } from "@/src/components/Interfaces";
...
const RoundBar = ({
    ...
    if (uniqueId === hostId && arcanes.length < arcaneCount) {
      ...
      for (let i = 0; i < arcaneCount + 1; i++) {
        const randomIndex = Math.floor(
          Math.random() * Object.entries(arcaneList).length
        );
        setArcaneColors((prevArcanes) => [
          ...prevArcanes,
          Object.values(arcaneList)[randomIndex],
        ]);
        setArcanes((prevArcanes) => [
          ...prevArcanes,
          Object.keys(arcaneList)[randomIndex],
        ]);
  ...
  useEffect(() => {
    if (uniqueId && uniqueId !== hostId && arcanes?.length > arcaneCount) {
      for (let i = 0; i < arcanes.length; i++) {
        const color = Object.entries(arcaneList).find(
          ([key, _value]) => key === arcanes[i]
        );
  ...
  return (
    <div
      id="fade-in"
      className="lg:mt-8 lg:mb-0  z-30 lg:static absolute top-2 "
    >
      <div className="relative">
        ...
        >
          ...
          >
            <path
              d={`M${round * fillWidth} 20H0V0H${round * fillWidth}V20Z`}
              fill="#B8AFE0"
            />
          </svg>
          <path
            d="M332.406 26.1667H2.59375V1.83342H332.406V26.1667Z"
            stroke="#E1DAFF"
            strokeWidth="6"
            strokeMiterlimit="10"
          />
