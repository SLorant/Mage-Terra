...
import { SquareState, ScoreBoardProps } from "../../../components/Interfaces";
import { mapLength } from "./boardcomponents/MapConfig";

export const ScoreBoard: FC<ScoreBoardProps> = memo(function ScoreBoard({
  uniqueId,
  playerInfos,
  needBoard,
  readBoards,
  ...
  const handleNextBoard = () => {
    if (readBoards) {
      if (Object.keys(readBoards).length - 1 === currentBoard)
        setCurrentBoard(0);
  ...
  const handlePrevBoard = () => {
    if (readBoards) {
      if (currentBoard === 0)
        setCurrentBoard(Object.keys(readBoards).length - 1);
      else setCurrentBoard(currentBoard - 1);
    }
  };
  ...
  if (readBoards) {
    firstPlayerSquares =
      Object.values(readBoards)[currentBoard]?.[0] ?? initialSquares;
    if (
      Object.keys(readBoards)[currentBoard] &&
      Object.keys(readBoards)[currentBoard] === uniqueId
    )
      boardName = "Your";
    ...
    >
      <button
        ...
        className={`${
          sbOpened
            ? scoreText
              ? " -translate-y-[548px] static z-50"
              : " -translate-y-[248px] static z-50"
            : ""
        }
      ...
      >
        ...
        >
          <path
            d="M47 19.9298L23.651 -1.02062e-06L0.236751 20.043L23.651 15.1738L47 19.9298Z"
            fill="white"
          />
        </svg>
      </button>
      <aside
        ...
        className={`${
          sbOpened
            ? "w-full transform-gpu duration-200 transition ease-in-out"
            : "hidden lg:flex"
        }
      ...
      >
        <div
          id="fade-in-fast"
          className={` ${
            scoreText ? "flex" : "hidden md:flex"
          } flex-col text-xl text-white w-full items-center text-center`}
        >
              ...
              <div
                ...
                className={`${
                  getRankByPlayerId(playerId) % 2 === 0
                    ? "bg-lightpurple lg:bg-grey"
                    : "bg-grey lg:bg-lightpurple"
        ...
        {needBoard && firstPlayerSquares && (
          <div className="absolute  w-full justify-center  bottom-12 lg:bottom-8 gap-4 z-50 flex h-[200px] lg:h-[165px] ">
            <button
              className="z-20 text-3xl text-white mb-6"
            ...
            <div className="flex flex-col items-center justify-center">
              <div className="h-[196px] w-[196px] lg:h-[165px] lg:w-[165px] grid grid-cols-7 grid-rows-7">
                {firstPlayerSquares.map(
                  ({ accepts, lastDroppedItem, hasStar }, squareIndex) => (
                    <MiniSquare
                      accept={accepts}
