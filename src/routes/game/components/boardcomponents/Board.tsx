...
import { BoardProps, SquareState } from "../../../../components/Interfaces";
import { rowLength, mapLength } from "./MapConfig";
import { TurnLeft, TurnRight } from "@/src/components/Vectors";
...
export const Board: FC<BoardProps> = memo(function Board({
  ...
  useEffect(() => {
    if (
      newSquares.length > 0 &&
      newSquares.some((square) => !square.lastDroppedItem)
    )
  ...
  function isValidNeighbour(
    ...
    if (Squares[index - targetIndex]) {
      if (!Squares[index - targetIndex].lastDroppedItem) return false;
        ...
        return Squares[index - targetIndex].lastDroppedItem.firstname.includes(
          firstname
        );
    } else return false;
  }

  function areNeighboursValid(
    index: number,
    firstname: string,
    secondname: string
  ): boolean {
    ...
    (
      ...
      if (
        ...
        const newSquares = update(Squares, {
          [fillIndex]: {
            lastDroppedItem: {
              $set: { firstname: secondname, img: secondimg },
            },
          },
  ...
  return (
    <div className="h-full mt-10 md:mt-6 w-full flex gap-4 relative flex-col">
      <div
        id="fade-in"
        className={`md:h-[${mapLength * 10}px] md:w-[${
          mapLength * 10
        }px] grid grid-cols-7 grid-rows-7`}
      >
        {Squares.map(({ accepts, lastDroppedItem, hasStar }, index) => (
          <Square
            ...
            onIsOverChange={(index, isOver) =>
              handleIsOverChange(index, isOver)
            }
            index={index}
            key={index}
      ...
      <div
        id="fade-in"
        className="ml-0 md:mt-16 md:ml-10 w-full md:w-[400px] md:h-[200px] h-[200px] flex justify-center items-center relative"
      >
        <div className="h-full w-full justify-center items-center text-white lg:ml-4 text-xl flex relative">
          <button
            className="absolute top-10 lg:top-14 left-20 md:left-72"
            onClick={handleLeftTurnClick}
          >
            <TurnLeft />
          </button>
          <div className="lg:ml-[460px] lg:mb-0 mb-28">
          ...
          <button
            className="absolute top-10 lg:top-14  right-20 md:-right-36"
            onClick={handleRightTurnClick}
          >
            <TurnRight />
          </button>
        </div>
