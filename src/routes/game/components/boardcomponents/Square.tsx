...
import { SquareProps } from "../../../../components/Interfaces";
...
export const Square: FC<SquareProps> = memo(function Square({
  ...
  for (let i = 0; i < droppedDominoes.length; i++) {
    if (index === droppedDominoes[i][0] && index + 1 === droppedDominoes[i][1])
      borderClass = "border-l-[1px] border-b-[1px] border-t-[1px]";
    if (index === droppedDominoes[i][1] && index - 1 === droppedDominoes[i][0])
      borderClass = "border-r-[1px] border-b-[1px] border-t-[1px]";
    if (
      index === droppedDominoes[i][0] &&
      index + rowLength === droppedDominoes[i][0]
    )
      borderClass = "border-r-[1px] border-l-[1px] border-t-[1px]";
    if (
      index === droppedDominoes[i][1] &&
      index - rowLength === droppedDominoes[i][0]
    )
      borderClass = "border-r-[1px] border-b-[1px] border-l-[1px]";
  }
  ...
  const ruins = [
    "/dominoes/ruin-01.webp",
    "/dominoes/ruin-02.webp",
    "/dominoes/ruin-03.webp",
  ];
  ...
  return (
    ...
    >
      ...
      {accept.includes("W") && (
        <div className={`h-auto w-auto  shadow-lg z-20 `}>
          <img
            src={`/dominoes/ruin-0${randomRuin}.webp`}
            alt="dropped"
            width={50}
            height={50}
            className="w-full h-full"
