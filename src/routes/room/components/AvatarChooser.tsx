...
import { PrevAvatar, NextAvatar } from "@/src/components/Vectors";
import { AvatarChooserProps } from "@/src/components/Interfaces";
...
const AvatarChooser = ({
  ...
  const [_, updatePlayerCount] = usePlayerStore((state) => [
    state.playerCount,
  ...
  const handleConfirmName = () => {
    if (inputName.length === 0) setError("Your name must contain characters");
      ...
      setError("Your name can be max 10 characters");
    else {
      const dataRef = ref(projectDatabase, `/${room}/${uniqueId}/Name`);
  ...
  return (
    <div
      id="fade-in"
      className="flex flex-col  items-center justify-center avatarchooser"
    >
      <div className="flex justify-center items-center  text-3xl">
        <button
          className={`prev mt-4 ${
            isClickedPrev
              ? "-translate-x-0.5 transition duration-200 ease-in-out"
        ...
        <img
          ...
          width={0}
          height={0}
          className="w-36 h-40 mainavatar"
        />
      ...
      >
        <div className="mt-4">
          ...
          <button
            ...
            } h-[40px] w-[100px] md:w-[120px] roombutton bg-lightpurple text-[#130242]
        ...
        {error && (
          <p className="text-lightpurple text-lg lg:text-xl  absolute -bottom-8">
            {error}
          </p>
        )}
      </div>
